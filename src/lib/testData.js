...
function generateClasses() {
  ...
  // Generate only future classes for the next 7 days
...
function generateUsers() {
  ...
  for (let i = 0; i < 20; i++) {
    ...
    users.push({
      ...
      memberSince: new Date(2023 + Math.random(), Math.floor(Math.random() * 12), Math.floor(Math.random() * 28) + 1).toISOString(),
...
function generateBookingsAndWaitlists(users, classes) {
  ...
  classes.forEach(c => { classBookingCounts[c.id] = 0; });
    ...
    for (let i = 0; i < Math.min(targetBookings, shuffledUsers.length); i++) {
      ...
      if (bookingsMade < capacity) {
        ...
        bookings.push({
          ...
          bookedAt: new Date(Date.now() - Math.random() * 7 * 24 * 60 * 60 * 1000).toISOString(), // Random time in last 7 days
          status: 'confirmed'
        });
        bookingsMade++;
        ...
        waitlists.push({
          ...
          userId: user.id,
          classId: classItem.id,
          joinedAt: new Date(Date.now() - Math.random() * 3 * 24 * 60 * 60 * 1000).toISOString() // Random time in last 3 days
        });
        waitlistEntries++;
      }
    }
...
export async function initializeTestData() {
  ...
  try {
    ...
    const { bookings, waitlists, classBookingCounts } = generateBookingsAndWaitlists(users, classes);
